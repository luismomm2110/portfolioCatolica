...
from Airports.repositories.repository import FakeAirportRepository
from CurrencyRate.gateways.gateways import FakeCurrencyRateGateway
from Flights.gateways.gateway_amadeus import FakeGateway
from Flights.models.model import FoundFlight
from Flights.services.services import find_all_flights_from_airports
from tests.utils import source, all_airports
...
def test_when_search_for_flights_with_all_inputs_then_should_return_correct_flights(fake_repository, fake_gateway):
    flights, _ = find_all_flights_from_airports(city_origin=city_source, iata_airports_destinations=iata_airports_destinations, departure=departure,
                                                airport_repository=fake_repository, flight_gateway=fake_gateway, currency_rate_mapping=currency_rate_mapping, max_price=price_in_brl)
    ...
    assert flights[0].total_price == price_in_brl
    ...
    assert flights[1].total_price == price_in_brl
    assert flights[0].currency == 'BRL'
